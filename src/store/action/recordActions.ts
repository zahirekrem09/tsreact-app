import { RecordType, RecordDispatch, RecordForm } from "../../types/record";
...
export const addRecord =
  (form: RecordForm) => async (dispatch: RecordDispatch) => {
    ...
    try {
      const response = await api().post<RecordType>("/records", form as any);
...
export const updateRecord =
  (form: Partial<RecordForm>, id: RecordType["id"]) =>
  async (dispatch: RecordDispatch) => {
    dispatch({ type: "UPDATE_RECORD_START" });
    try {
      const response = await api().put<RecordType>(
        "/records/" + id,
        form as any
      );
...
export const deleteRecord =
  (id: RecordType["id"]) => async (dispatch: RecordDispatch) => {
